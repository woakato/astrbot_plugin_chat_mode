...
### 新增 (Added)
- **上一图参考接力**：生成插图时自动把上一张成功插图作为参考图传给 NAI 插件（按其「参考图使用模式」vibe/img2img 路由，强度走插件默认值），在 Tag 提示词锚点之外叠加像素级的角色形象与画风延续。
- **参考图缺失兜底**：首图尚无参考图、上次生成失败、或参考图文件已被清理时，自动回退为纯文生图并记录日志；提示词锚点与参考图相互独立降级，链条不会因任何单次失败中断。
